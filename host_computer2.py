#@StartTime     :7/6/2021
#@Auther        :Chengda Wen
#@Software      :PyCharm
#@File          :host_computer2(Equivalent to Client)
#@Task          :nwpu_summercamp

from socket import *
import time
import random
import math
from SM3 import *




# Connect to host_computer1:
print("Time："+time.strftime('%m/%d/%Y %H:%M:%S', time.localtime(time.time())))
print('Connecting...')
Key_SM4 = '0123456789ABCDEF'
Host = 'localhost'
Port = 6666
Buff = 1024
Addr = (Host, Port)

t = 3
host2 = socket(AF_INET, SOCK_STREAM)
host2.connect(Addr)
print(time.strftime('%m/%d/%Y %H:%M:%S  ', time.localtime(time.time()))+' host_computer2 has been connected.')

# Sending command "Distribution key" to host_computer1:
# host_computer2 receives RSA_Ke from host_computer1
print(time.strftime('%m/%d/%Y %H:%M:%S  ', time.localtime(time.time()))+' Prepared for Receiving RSA_Key!')
host2.send('Distribution key'.encode('utf-8'))
# @@ host2_step1. receive rsa_ke from host1:
print('Host_computer2 Step 1.  To receive RSA_Key from host_computer1:')
RSA_Kd = int(host2.recv(32).decode())
RSA_Ke = int(host2.recv(32).decode())  # @ get host_computer1's RSA_Ke
n = int(host2.recv(32).decode())

print('Host_computer2 has received RSA_Ke from host_computer1.\n')
# @@ host2_step2. encrypt sm4_k with rsa_ke:
print('Host_computer2 Step 2.  Encrypt SM4_Key with RSA_Ke:')
# 十进制转为十六进制
def decToHex(n):
    return hex(int(n, 10))[2:]
# 十六进制转为十进制
def hexToDec(n):
    return int(n, 16)
# 平方求模
def getMod(a, e, m):
    result = 1
    while e != 0:
        if e & 1 == 1:
            result = result * a % m
        e >>= 1
        a = a * a % m
    return result
# 加密函数
def encrypt(m, e, n):
    assert m < n, "m must be shorter than n"
    return getMod(m, e, n)
# 64bit(16位）一组，进行RSA加密(p为16进制，e、n为10进制，输出c为10进制）
def rsa_encrypt(p, e, n):
    n_str = decToHex(str(n))
    c = ""
    while len(p) >= 16:
        tmp = decToHex(str(encrypt(hexToDec(p[0:16]), e, n)))
        if len(tmp) < len(n_str):
            tmp = (len(n_str) - len(tmp)) * '0' + tmp
        c += tmp
        p = p[16:]

    if len(p) != 0:
        tmp = decToHex(str(encrypt(hexToDec(p[0:16]), e, n)))
        if len(tmp) < len(n_str):
            tmp = (len(n_str) - len(tmp)) * '0' + tmp
        c += tmp

    return c

# print(tempcipher)

# tempcipher = str(encrypt(Key_SM4.encode('utf8'), RSA_Ke, n))
#print(tempcipher)
print(time.strftime('%m/%d/%Y %H:%M:%S  ', time.localtime(time.time()))+' send encrypted sm4_key')
host2.send(str(Key_SM4).encode('utf-8'))

# @@ host2_step3. gets SM2_Key(Ke,Kd) and sends Kd to host_computer1:
# 扩展欧几里得算法求逆元
def get_gcd(a, b):
    if(b==0):
        return 1,0,a
    else:
        x,y,gcd = get_gcd(b,a%b)
        x,y = y,(x-(a//b)*y)
        return x,y,gcd
# 两点加法
def add_point(x1,y1,x2,y2,p):
    if(x1=='O' and y1=='O'):
        return x2,y2
    elif(x2=='O' and y2=='O'):
        return x1,y1
    elif(x1==x2 and y2==((-1)*y1)%p):
        x3 = 'O'
        y3 = 'O'
        return x3,y3
    else:
        inv,y,gcd = get_gcd(x2-x1,p)
        lbd = ((y2-y1)*inv)%p
        x3 = (lbd**2-x1-x2)%p
        y3 = (lbd*(x1-x3)-y1)%p
        return x3,y3

# 倍点算法
def multiply2_point(x1,y1,a,p):
    if(x1=='O' and y1=='O'):
        return x1,y1
    else:
        inv,y,gcd = get_gcd(2*y1,p)
        lbd = ((3*(x1**2)+a)*inv)%p
        x3 = (lbd**2-2*x1)%p
        y3 = (lbd*(x1-x3)-y1)%p
        return x3,y3
# k倍点算法
def multiplyk_point(Px,Py,k,a,p):
    k = bin(k)[2:]
    Qx = 'O'
    Qy = 'O'
    for j in range(len(k)):
        Qx,Qy = multiply2_point(Qx,Qy,a,p)
        if(k[j]=='1'):
            Qx,Qy = add_point(Qx,Qy,Px,Py,p)
    return Qx,Qy

# 验证公钥满足条件
def key_statisfy(n,Px,Py,a,b,p):
    # P不能是无穷远点
    if(Px=='O' or Py=='O'):
        return False
    # P必须是Fq中的元素
    if(Px<0 or Py<0 or Px>p-1 or Py>p-1):
        return False
    # P满足椭圆曲线方程
    left = (Py**2)%p
    right = (Px**3+a*Px+b)%p
    if(left!=right):
        return False
    # [n]P为无穷远点
    nPx,nPy = multiplyk_point(Px,Py,n,a,p)
    if(nPx!='O' or nPy!='O'):
        return False
    return True

# 产生公钥
def gen_keypair(n,Gx,Gy,a,b,p):
    d = random.randint(1,n-1)
    Px,Py = multiplyk_point(Gx,Gy,d,a,p)
    while(not key_statisfy(n,Px,Py,a,b,p)):
        d = random.randint(1,n-1)
        Px,Py = multiplyk_point(Gx,Gy,d,a,p)
    return d,Px,Py

# y^2=x^3+ax+b
# 推荐系统参数
p = int('FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00000000FFFFFFFFFFFFFFFF',base=16)
a = int('FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00000000FFFFFFFFFFFFFFFC',base=16)
b = int('28E9FA9E9D9F5E344D5A9E4BCF6509A7F39789F515AB8F92DDBCBD414D940E93',base=16)
n = int('FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFF7203DF6B21C6052B53BBF40939D54123',base=16)
Gx = int('32C4AE2C1F1981195F9904466A39C9948FE30BBFF2660BE1715A4589334C74C7',base=16)
Gy = int('BC3736A2F4F6779C59BDCEE36B692153D0A9877CC62A474002DF32E52139F0A0',base=16)
# 产生公私钥对
d, Px, Py = gen_keypair(n, Gx, Gy, a, b, p)
host2.send(hex(d).encode('utf-8'))
host2.send(hex(Px).encode('utf-8'))
host2.send(hex(Py).encode('utf-8'))
#host2.send(hex(Py).encode('utf-8'))

#print('d:', hex(d), '\nPx:', hex(Px), '\nPy:', hex(Py))
# @@ host2_step4. SM2_Sign the tempcipher(SM4_Key):
# 消息字符串转比特串
def msg2bit(msg):
    res = ''
    for c in msg:
        a = ord(c)
        res += bin(a)[2:].zfill(8)
    return res
# 比特串转域元素
def bit2Fq(b):
    for i in range(len(b)):
        if (b[i] == '1'):
            b = b[i:]
            break
    return int(b, base=2)
# 比特串转消息字符串
def bit2msg(b):
    res = ''
    for i in range(int(len(b)/8)):
        cbit = b[i*8:(i+1)*8]
        res += chr(int(cbit,base=2))
    return res
# 域元素到比特串的转换
def Fq2bit(alpha,p):
    t = math.ceil(math.log(p,2))
    M = bin(alpha)[2:]
    while(len(M)%8!=0 or len(M)!=t):
        M = '0' + M
    #print(M,len(M))
    return M
# KDF combined with SM3
def KDF(Z, klen):
    v = 256
    ct = 1
    Ha = {}
    for i in range(1, math.ceil(klen / v) + 1):
        Ha[i] = SM3_digest(Z + bin(ct)[2:].zfill(32))
        ct += 1
    index = math.ceil(klen / v)
    Haa = ''
    if (math.ceil(klen / v) == klen / v):
        Haa = Ha[index]
    else:
        Haa = Ha[index][:klen - (v * math.floor(klen / v))]
    K = ''
    for i in range(1, math.ceil(klen / v)):
        K += Ha[i]
    K += Haa
    return K
# 点到比特串转换
def point2bit(xp,yp,p):
    PC = '00000100'  # 选择不压缩模式
    xp_bit = Fq2bit(xp,p)
    yp_bit = Fq2bit(yp,p)
    return PC+xp_bit+yp_bit

# KDF combined with SM3
def KDF(Z,klen):
    v = 256
    ct = 1
    Ha = {}
    for i in range(1,math.ceil(klen/v)+1):
        Ha[i] = SM3_digest(Z+bin(ct)[2:].zfill(32))
        ct += 1
    # klen/v is integer
    index = math.ceil(klen/v)
    Haa = ''
    if(math.ceil(klen/v)==klen/v):
        Haa = Ha[index]
    else:
        Haa = Ha[index][:klen-(v*math.floor(klen/v))]
    K = ''
    for i in range(1,math.ceil(klen/v)):
        K += Ha[i]
    K += Haa
    return K
# 按位异或
def Xor(a,b):
    result =''
    if len(a)!=len(b):
        return False
    for i in range(len(a)):
        if a[i]==b[i]:
            result += '0'
        else:
            result += '1'
    return result
# bit str M
def SM2_encrypt(M,n,Gx,Gy,a,b,p,Px,Py):
    #print('SM2 ENCRYPTION')
    klen = len(M)
    # A1
    k = random.randint(1,n-1)
    # A2
    x1,y1 = multiplyk_point(Gx,Gy,k,a,p)
    C1 = point2bit(x1,y1,p)
    # A3
    h = math.floor(((math.sqrt(p)+1)**2)/n)
    Sx,Sy = multiplyk_point(Px,Py,h,a,p)
    if(Sx=='O' or Sy=='0'):
        return False
    # A4
    x2,y2 = multiplyk_point(Px,Py,k,a,p)
    x2_bit = Fq2bit(x2,p)
    y2_bit = Fq2bit(y2,p)
    # A5
    t = KDF(x2_bit+y2_bit,klen)
    if(int(t,base=2)==0):
        return False
    # A6
    C2 = Xor(M, t)
    # A7
    C3 = SM3_digest(x2_bit+M+y2_bit)
    # A8
    C = [C1, C2, C3]
    host2.send(str(C1).encode('utf-8'))
    host2.send(str(C2).encode('utf-8'))
    host2.send(str(C3).encode('utf-8'))
    return C
# SM2_Encrpytion
#print(tempcipher)
plain_text = msg2bit(Key_SM4)
cipher_text = SM2_encrypt(plain_text, n, Gx, Gy, a, b, p, Px, Py)
hexresult = ''
for c in cipher_text:
    hexresult += hex(int(c, base=2))
print('cipher:'+str(cipher_text))
#host2.send(json.dumps(cipher_text).encode('utf-8'))

# 84 lines RSA ENCRYPT TEST
tempcipher = rsa_encrypt(Key_SM4, RSA_Ke, n)
'''
def text_save(filename, data):#filename为写入txt文件的路径，data为要写入数据列表.
  file = open(filename,'a')
  for i in range(len(data)):
    s = str(data[i]).replace('[','').replace(']','')#去除[],这两行按数据不同，可以选择
    s = s.replace("'",'').replace(',','') +'\n'  #去除单引号，逗号，每行末尾追加换行符
    file.write(s)
  file.close()
  print("保存文件成功")
'''

f = open("SM2_cilpherlist.txt", "w+")
#for l in cipher_text:
#    f.write(str(l) + '\n')
#f.write('----------\n')
# 将列表的 [] , 全部去掉再写入



def SM2_decrypt(C, n, Gx, Gy, a, b, p, d):
    # print('SM2 DECRYPTION')
    C1 = C[0]
    C2 = C[1]
    C3 = C[2]
    klen = len(C2)

    # B1
    PC = C1[:8]  # PC=04
    bit_len = int((len(C1) - 8) / 2)
    x1 = bit2Fq(C1[8:8 + bit_len])
    y1 = bit2Fq(C1[8 + bit_len:])
    left = (y1 ** 2) % p
    right = (x1 ** 3 + a * x1 + b) % p
    if (left != right):
        return False
    # B2
    h = math.floor(((math.sqrt(p) + 1) ** 2) / n)
    Sx, Sy = multiplyk_point(Px, Py, h, a, p)
    if (Sx == 'O' or Sy == '0'):
        return False
    # B3
    x2, y2 = multiplyk_point(x1, y1, d, a, p)
    x2_bit = Fq2bit(x2, p)
    y2_bit = Fq2bit(y2, p)
    # B4
    t = KDF(x2_bit + y2_bit, klen)
    if (int(t, base=2) == 0):
        return False
    # B5
    MM = Xor(C2, t)
    # B6
    u = SM3_digest(x2_bit + MM + y2_bit)
    if (u != C3):
        return False
    # B7
    return MM

# SM2_Decryption
decrypt_text = SM2_decrypt(cipher_text, n, Gx, Gy, a, b, p, d)
decrypt_text = bit2msg(decrypt_text)


def decrypt(c, d, n):
    return getMod(c, d, n)


def rsa_decrypt(c, d, n):
    n_hex = decToHex(str(n))
    m = ""
    num = len(n_hex)
    while len(c) > 0:
        x = hexToDec(c[0: num])
        c = c[len(n_hex):]
        tmp = decrypt(x, d, n)
        if len(decToHex(str(tmp))) < 16 and len(c) > 0:
            m += (16 - len(decToHex(str(tmp)))) * '0'
        m += decToHex(str(tmp))
    return m
aes_key = rsa_decrypt(tempcipher, int(RSA_Kd), n)
print(aes_key)


print('解密得到明文：', decrypt_text)
f.write(decrypt_text)
f.close()
f = open("SM2_cilpherlist.txt", "r")
file = f.readlines()
print(file)


